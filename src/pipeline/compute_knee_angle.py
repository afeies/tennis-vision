...
OUTPUT_CSV = Path(f"data/processed/csv/{SERVE_NAME}_knee_angle.csv")
...
with open(INPUT_CSV, "r") as f_in, open(OUTPUT_CSV, "w", newline="") as f_out:
    ...
    writer.writerow(["frame", "knee_angle_deg"])

    for row in reader:
        frame = int(row["frame"])

        # Right leg (serving side typically)
        hip = (float(row["right_hip_x"]), float(row["right_hip_y"]))
        knee = (float(row["right_knee_x"]), float(row["right_knee_y"]))
        ankle = (float(row["right_ankle_x"]), float(row["right_ankle_y"]))

        # Knee angle: hip -> knee -> ankle
        # Measures knee flexion/extension
        angle = angle_between(hip, knee, ankle)

        if angle is not None:
            writer.writerow([frame, angle])

print(f"Saved knee angle time series to: {OUTPUT_CSV}")
