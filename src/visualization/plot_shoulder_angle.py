...
def load_serve_data(serve_name):
    csv_path = Path(f"data/processed/csv/{serve_name}_shoulder_angle.csv")
    frames = []
    angles = []
    with open(csv_path, "r") as f:
        ...
        for row in reader:
            ...
            angles.append(float(row["shoulder_angle_deg"]))
    return frames, angles

def load_events(serve_name):
    """Load shoulder events if they exist (future implementation)"""
    csv_path = Path(f"data/processed/csv/{serve_name}_shoulder_events.csv")
    events = {}
    try:
        with open(csv_path, "r") as f:
            reader = csv.DictReader(f)
...
if args.all:
    for serve in SERVES:
        try:
            ...
            if "max_external_rotation" in events:
                ax.axvline(events["max_external_rotation"], color='red', linestyle=':', alpha=0.3, linewidth=1)
    ...
    ax.legend()
    ax.set_title("Shoulder Angle Over Time - All Serves")
    ...
    ax.plot(frames, angles, label="Shoulder angle")

    # Plot event markers if they exist
    if "max_external_rotation" in events:
        ax.axvline(events["max_external_rotation"], color='red', linestyle='--', linewidth=2, label='Max external rotation')
    if "contact" in events:
        ax.axvline(events["contact"], color='green', linestyle='--', linewidth=2, label='Contact')

    ax.legend()
    ax.set_title(f"Shoulder Angle Over Time - {serve_name}")
else:
    print("Usage: python plot_shoulder_angle.py --serve feies.mov")
    print("       python plot_shoulder_angle.py --all")
    exit(1)
