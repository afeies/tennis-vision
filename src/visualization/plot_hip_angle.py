...
def load_serve_data(serve_name):
    csv_path = Path(f"data/processed/csv/{serve_name}_hip_angle.csv")
    frames = []
    angles = []
    with open(csv_path, "r") as f:
        ...
        for row in reader:
            ...
            angles.append(float(row["hip_angle_deg"]))
    return frames, angles

def load_events(serve_name):
    """Load hip events if they exist (future implementation)"""
    csv_path = Path(f"data/processed/csv/{serve_name}_hip_events.csv")
    events = {}
    try:
        with open(csv_path, "r") as f:
            reader = csv.DictReader(f)
...
if args.all:
    for serve in SERVES:
        try:
            ...
            if "hip_rotation" in events:
                ax.axvline(events["hip_rotation"], color='red', linestyle=':', alpha=0.3, linewidth=1)

        except FileNotFoundError:
            print(f"Warning: {serve} data not found, skipping")
    ax.legend()
    ax.set_title("Hip Angle Over Time - All Serves")
    ...
    ax.plot(frames, angles, label="Hip angle")

    # Plot event markers if they exist
    if "hip_rotation" in events:
        ax.axvline(events["hip_rotation"], color='red', linestyle='--', linewidth=2, label='Hip rotation')

    ax.legend()
    ax.set_title(f"Hip Angle Over Time - {serve_name}")
else:
    print("Usage: python plot_hip_angle.py --serve feies.mov")
    print("       python plot_hip_angle.py --all")
    exit(1)
